...
function allow(mode, checkPermissionsForDirectory) {
  return async function allowHandler(req, res, next) {
    ...
    let resourcePath =
      res && res.locals && res.locals.path ? res.locals.path : req.path;

    if (resourcePath === "/download") {
      resourcePath = req.query.path;
    }
    ...
    const trustedOrigins = [ldp.resourceMapper.resolveUrl(req.hostname)].concat(
      ldp.trustedOrigins
    );
    if (ldp.multiuser) {
    ...
    if (mode === "Read" && (resourcePath === "" || resourcePath === "/")) {
      ...
      try {
        representationPath = await ldp.resourceMapper.mapUrlToFile({
    ...
    debug(
      `${mode} access denied to ${userId || "(none)"}: ${error.status} - ${
        error.message
      }`
    );
    next(error);
