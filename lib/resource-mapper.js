...
class ResourceMapper {
  ...
  async mapFileToUrl({ path, hostname }) {
    ...
    const url = `${this.resolveUrl(hostname)}${pathname
      .split(pathUtil.sep)
  ...
  async mapUrlToFile({
    ...
    const acceptedTypes = url.headers ? url.headers.accept : undefined;
    const htmlWanted = acceptedTypes
    ...
    if (createIfNotExists) {
      ...
      if (isIndex) {
        if (contentType !== this._indexContentType) {
          throw new Error(
            `Index file needs to have ${this._indexContentType} as content type`
          );
        }
      ...
      if (
        !isFolder &&
        this._getContentTypeFromExtension(path) !== contentType
      ) {
  ...
  _getContentTypeFromExtension(path) {
    ...
    return (
      (extension && this._types[extension[1].toLowerCase()]) ||
      this._defaultContentType
    );
  }
  ...
  _addContentTypeExtension(path, contentType) {
    ...
    if (contentTypeFromExtension !== contentType) {
      ...
      const newExtension =
        contentType in extensions ? extensions[contentType][0] : "unknown";
