...
class LDP {
  ...
  async listContainer(container, reqUri, containerData, hostname) {
    ...
    try {
      // add container stats
      await ldpContainer.addContainerStats(
        this,
        reqUri,
        container,
        resourceGraph
      ...
      await Promise.all(
        files.map(async (file) => {
          const { url: fileUri } = await this.resourceMapper.mapFileToUrl({
            path: join(container, file),
            hostname,
          });
          return await ldpContainer.addFile(
            this,
            resourceGraph,
            reqUri,
            fileUri,
  ...
  async post(
    ...
    containerPath,
    stream,
    { container, slug, extension, contentType }
  ) {
    const ldp = this;
    ...
    let resourceUrl = await ldp.getAvailableUrl(hostname, containerPath, {
      slug,
      extension,
    });
    ...
    if (container) {
      ...
      resourceUrl = `${resourceUrl}${resourceUrl.endsWith("/") ? "" : "/"}${
        ldp.suffixMeta
      }`;
  ...
  async put(url, stream, contentType) {
    ...
    if (
      (url.url || url).endsWith(this.suffixAcl) &&
      contentType !== this.aclContentType
    ) {
    ...
    try {
      ...
      isOverQuota = await overQuota(
        this.resourceMapper.resolveFilePath(hostname),
        this.serverUri
      );
    } catch (err) {
  ...
  async graph(url, baseUri, contentType) {
    ...
    return new Promise((resolve, reject) => {
      ...
      $rdf.parse(body, graph, baseUri, contentType, (err) =>
        err ? reject(err) : resolve(graph)
      );
    });
  }

  async get(options, searchIndex = false) {
    ...
    try {
      ...
      throw error(404, "Can't find file requested: " + options.path);
    }
    ...
    if (stats.isDirectory()) {
      const { url: absContainerUri } = await this.resourceMapper.mapFileToUrl({
        path,
        hostname: options.hostname,
      });
      const metaFile = await this.readContainerMeta(absContainerUri).catch(
        () => ""
      ); // Default to an empty meta file if it is missing
      let data;
      try {
  ...
  async deleteContainer(directory) {
    ...
    if (
      list.some(
        (file) =>
          !file.endsWith(this.suffixMeta) && !file.endsWith(this.suffixAcl)
      )
    ) {
  ...
  async deleteResource(path) {
    try {
      return await withLock(path, { mustExist: false }, () =>
  ...
  async getAvailableUrl(
    hostname,
    containerURI,
    { slug = uuid.v1(), extension }
  ) {
    let requestUrl = this.resourceMapper.resolveUrl(hostname, containerURI);
    requestUrl = requestUrl.replace(/\/*$/, "/");

    const { path: containerFilePath } = await this.resourceMapper.mapUrlToFile({
      url: requestUrl,
    });
    let fileName =
      slug.endsWith(extension) ||
      slug.endsWith(this.suffixAcl) ||
      slug.endsWith(this.suffixMeta)
        ? slug
        : slug + extension;
    if (
      await promisify(fs.exists)(utilPath.join(containerFilePath, fileName))
    ) {
  ...
  getTrustedOrigins(req) {
    let trustedOrigins = [this.resourceMapper.resolveUrl(req.hostname)].concat(
      this.trustedOrigins
    );
    if (this.multiuser) {
