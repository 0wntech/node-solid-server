...
async function handler(req, res, next) {
  ...
  if (ldp.live) {
    res.header(
      "Updates-Via",
      ldp.resourceMapper.resolveUrl(req.hostname).replace(/^http/, "ws")
    );
  }
  ...
  if (path === "/download") {
    options.path = req.query.path ? req.query.path : "/";
  }
  ...
  try {
    ret = await ldp.get(
      options,
      req.accepts(["html", "turtle", "rdf+xml", "n3", "ld+json"]) === "html" &&
        path !== "/download"
    );
  } catch (err) {
  ...
  if (requestedType && requestedType.includes("text/html")) {
    const { path: filename } = await ldp.resourceMapper.mapUrlToFile({
      url: options,
      searchIndex: path !== "/download",
    });

    let mimeTypeByExt = mime.lookup(_path.basename(filename));
    let isHtmlResource = mimeTypeByExt && mimeTypeByExt.includes("html");
    let useDataBrowser =
      container || (RDFs.includes(contentType) && !isHtmlResource);

    if (path === "/download") {
      if (container) {
        let containerName = filename.split("/");
        containerName = containerName[containerName.length - 1];
        res.zip({
          files: [{ path: filename, name: containerName }],
    ...
    if (useDataBrowser) {
      let typeToConvert = possibleRDFType ? possibleRDFType : "text/turtle";
      typeToConvert =
        typeToConvert !== "text/html" ? typeToConvert : "text/turtle";
      const data = await translate(stream, baseUri, contentType, typeToConvert);
      console.log("Responding with html");
      res.set("Content-Type", "text/html");
      res.send(`<pre>${escapeHTML(data)}</pre>`);
      return;
    } else if (stream) {
  ...
  try {
    ...
    debug(
      req.originalUrl + " translating " + contentType + " -> " + possibleRDFType
    );
...
async function globHandler(req, res, next) {
  ...
  const folderPath = (
    await ldp.resourceMapper.mapUrlToFile({
      url: folderUrl,
      searchIndex: false,
    })
  ).path;
      ...
      matches.map(
        (match) =>
          new Promise(async (resolve, reject) => {
            const urlData = await ldp.resourceMapper.mapFileToUrl({
              path: match,
              hostname: req.hostname,
            });
