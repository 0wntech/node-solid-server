...
async function handler (req, res, next) {
  ...
  let ret
  try {
    ret = await ldp.get(options, searchIndex=(requestedType ? requestedType.includes('text/html') && path !== '/download' : false));
  } catch (err) {
  ...
  if (requestedType && requestedType.includes('text/html')) {
    const { path: filename } = await ldp.resourceMapper.mapUrlToFile({ url: options, searchIndex: (path !== '/download') })

    let mimeTypeByExt = mime.lookup(_path.basename(filename))
    let isHtmlResource = mimeTypeByExt && mimeTypeByExt.includes('html')
    let useDataBrowser = container || RDFs.includes(contentType) && !isHtmlResource
    
    if(path === '/download'){
      if(container){
	let containerName = filename.split('/');
	containerName = containerName[containerName.length - 1];
	res.zip({files: [{path: filename, name: containerName}], filename: containerName + '.zip'});
        return
      }
    ...
    if (useDataBrowser) {
      ...
      typeToConvert = typeToConvert !== 'text/html' ? typeToConvert : 'text/turtle' 
